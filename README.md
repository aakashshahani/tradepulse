...
A real-time streaming data pipeline for crypto market data. Built in phases:
Coinbase WebSocket → Kafka → Spark structured streaming → Postgres → Streamlit.

## Status

- **Phase 1 — Infrastructure & schema.** Kafka (KRaft) + Postgres 18 via Docker
  Compose, with the pipeline schema created on first Postgres startup.
- **Phase 2 — Coinbase bridge producer.** A Python service that republishes live
  Coinbase trades onto Kafka. ← current

Spark job and Streamlit dashboard are later phases (not implemented yet).
...
├── producer/            # Phase 2: Coinbase → Kafka producer
│   ├── producer.py
│   ├── requirements.txt
│   ├── Dockerfile
...
| `producer`   | built locally        | Coinbase Exchange WS → `trades.raw`.                           |

### Kafka listeners (host vs. in-network)

The broker advertises two addresses because a single one can't serve both paths
(the address a client is told to reconnect on must be resolvable from *that*
client):

- **In-network containers** use `kafka:9092` (the `INTERNAL` listener). This is
  what the producer connects to.
- **The host machine** uses `localhost:29092` (the `HOST` listener, published to
  the host). Note: **29092**, not 9092.

`docker compose exec kafka …` runs *inside* the broker container, so it uses
`localhost:9092` — that's fine and unaffected by the above.
...
Each value matches `schemas/trades.raw.schema.json`, e.g.:

```json
{"symbol":"BTC-USD","price":58699.73,"size":8e-08,"side":"buy","trade_id":1047769595,"ts":"2026-07-01T10:12:39.996920Z"}
```

**Check the producer's health** (driven by a heartbeat file it refreshes on every
message from the feed; goes `unhealthy` if the feed stalls):
...
Tests aren't shipped in the runtime image; run them in the producer image with
the source mounted:
...
## Verifying Phase 1 (infra & schema)
...
> If you changed `POSTGRES_USER` / `POSTGRES_DB` in `.env`, substitute them above.

## Tearing down

```bash
docker compose down          # stop containers, keep data volume
docker compose down -v       # also remove the Postgres volume (wipes data +
                             # re-runs init.sql on next startup)
```

> `sql/init.sql` runs **only** when the Postgres data volume is empty. If you
> change the schema, run `docker compose down -v` to re-initialize.
>
